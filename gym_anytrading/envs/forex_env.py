...
class ForexEnv(TradingEnv):
    ...
    def _process_data(self):
        ...
        signal_features = np.column_stack((pd.concat([(self.df.iloc[:,:4]-self.df.iloc[:,:4].mean())/self.df.iloc[:,:4].std(), (self.df.iloc[:,4]-self.df.iloc[:,4].mean())/self.df.iloc[:,4].std()], axis=1).to_numpy()))
